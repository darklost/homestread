...
class Homestead
  def self.configure(config, settings)
    ...
    if settings.has_key?('databases')
      ...
      enabled_databases.push 'mariadb'
      enabled_databases.push 'postgres'
      ...
      settings['databases'].each do |db_type|
       
        db_type_name = db_type.keys[0]
       
        #检查不存在
        unless enabled_databases.include? db_type_name
          config.vm.provision "shell", inline: "ignore db_type_name: #{db_type_name}  \n"
          next
        end

        db_shell_type_name=db_type_name

        #确认脚本路径
        ## mariadb 使用mysql的创建脚本
        if db_shell_type_name =="mariadb"
          db_shell_type_name = "mysql"
        end 
        ##拼接数据库创建脚本
        db_create_shell_path=script_dir + "/create-"+db_shell_type_name+".sh"
        if !File.exist? File.expand_path(db_create_shell_path)
            config.vm.provision "shell", inline: "echo Invalid db_create_shell_path: #{db_create_shell_path} \n"
            next
        end
        ##数据库名列表
        dbs = db_type[db_type_name]

        #遍历所需创建数据库名
        dbs.each do |db|
            #shell 
            config.vm.provision "shell", inline: "echo db_type_name: #{db_type_name} db: #{db} \n"
            db_create_shell_name="Creating "+db_type_name+" Database: " +db
            ##执行shell
            config.vm.provision 'shell' do |s|
              s.name = db_create_shell_name
              s.path = db_create_shell_path
              s.args = [db]
            end
        end
      end  
